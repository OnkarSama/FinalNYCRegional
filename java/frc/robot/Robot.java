...
public class Robot extends TimedRobot {
  ...
  // Pigeon2 pigeonYe = new Pigeon2(0, "rio");
  // int loopCount = 0;
  ...
  public void robotPeriodic() {
    ...
    seesTarget = tv.getBoolean(true);
    // double y = ty.getDouble(0.0);
    // double area = ta.getDouble(0.0);

    SmartDashboard.putNumber("LimelightX", limelightTx);
  ...
  private double trackingKp = -0.065;
  private double min_command = -0.15;
  private double allowedError = 1.5;
  ...
  public void teleopPeriodic() {
    ...
    if (match.color == kBlueTarget) {
      colorString = "Blue";
      if(!controller2.getSquareButton())
        led.set(.87);
    ...
    if (controller2.getPOV() == 0) { // changed to pov dpad up test tmr
      double heading_error = -limelightTx;
      double steering_adjust = 0.0;
      if (limelightTx > allowedError) {

        steering_adjust = trackingKp * heading_error - min_command;

      } else if (limelightTx < allowedError) {

        steering_adjust = trackingKp * heading_error + min_command;

      } else {
        // Target in range
        if(seesTarget && Math.abs(heading_error) < allowedError) {
    ...
    if(controller2.getR1Button()){
      led.set(-.11);
      lift.set(1);
    } else if(controller2.getL1Button()){
      lift.set(-1);
      led.set(-.11);
    } else {
      lift.set(0);
